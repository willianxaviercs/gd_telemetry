...
- `EDGE_DAEMON_POLL_INTERVAL_SECONDS`
- `REDIS_HOST`
- `REDIS_PORT`
- `REDIS_STREAM`
...
- [runtime/bin/start.sh](/home/xavier/programming/agent-coding/gundam/runtime/bin/start.sh) starts simulators and edge-daemons in the background and stores PID files under `runtime/run/`
- [runtime/bin/stop.sh](/home/xavier/programming/agent-coding/gundam/runtime/bin/stop.sh) stops background simulators and edge-daemons tracked in `runtime/run/`
...
- start one simulator and one edge-daemon per device
- keep writing simulator logs and edge-daemon logs inside each device directory
